...
$sql_public="SELECT distinct(ketab_num),ketab_date  from studies_all WHERE date(add_date) >= '$start' AND date(add_date) <= '$end' AND jeha = '$jeha_profile' AND details_type = '$details_type' ORDER BY ketab_num ASC";
                ...
                <td colspan="" style="border:none;">
                    <div id="container">
                        ...
                        <div id="center" style="font-family: 'Hacen Liner Screen'; color: black; width:300px">
                            <a>فهرس البريد الإلكتروني الصادر (المسح الأمني)
                            <br>
                            تاريخ التصدير من 
                            <?php echo $start; ?>
                            إلى
           ...
           <!--  </thead> -->
            <tbody>
                ...
                while($row_public = mysqli_fetch_assoc($sql_public_result)) { 
                    ...
                    //count studies_all
                    $sql_studies_all_count="SELECT count(id) as count_studies_all from `studies_all` WHERE  ketab_num=".$row_public['ketab_num']." 
                    AND ketab_date = '".$row_public['ketab_date']."' AND jeha = '$jeha_profile' AND details_type = '$details_type'  ORDER BY ketab_num ASC"; 
                    $sql_studies_all_count_result = mysqli_query($conn, $sql_studies_all_count);
                    if (mysqli_num_rows($sql_studies_all_count_result)> 0) {
                    $row_studies_all_count = mysqli_fetch_assoc($sql_studies_all_count_result);
                    $studies_all_count = $row_studies_all_count['count_studies_all'];
                    }else{
                        $studies_all_count = 0;
                    }
                    ...
                    ?>
                     ...
                     <td><?php echo $studies_all_count; ?></td>
